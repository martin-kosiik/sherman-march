...
names_lm_1930 <- lm_robust(I(share_southern_names_1930 * 100) ~ march + pslave1860 + land.ineq1860 + log(acimp1860) + log(fvalpac1860) + rail1860 + log(totpop1860), data = county_data)

names_lm_1880 <- lm_robust(I(share_southern_names_1880 * 100) ~ march + pslave1860 + land.ineq1860 + log(acimp1860) + log(fvalpac1860) + rail1860 + log(totpop1860), data = county_data)

names(county_data)

streets_lm <- lm_robust(conf_streets ~ march + pslave1860 + land.ineq1860 + log(acimp1860) + log(fvalpac1860) + rail1860 + log(totpop1860) + state.abb , data = county_data)
...
note_text <- "All the control variables show their value as of 1860 (i.e. slave share in 1860 etc.). The standard errors are in the parentheses. State fixed effects are included in all speciifications. Monuments is a an indicator variable that equals one if a Confederate monument was present in a county in 2019. Lynch rate is defined as the number of lynchings in a county from "

modelsummary(list('Monuments' = monuments_lm, 'Lynch rate' = lynchrate_lm, 
                  "First names (1880)" = names_lm_1880, "First names (1930)" = names_lm_1930,
                  "Street names" = streets_lm), 
