...
table1::label(county_data$pdem1860) <- "Democrats voteshare in 1860"
table1::label(county_data$pdem1872) <- "Democrats voteshare in 1872"
table1::label(county_data$pdem1900) <- "Democrats voteshare in 1900"
...
table1::label(county_data$pct_conf_names_1930) <- "Conf. first names share (%)-1930 census"
table1::label(county_data$pct_conf_names_1880) <- "Conf. first names share (%)-1880 census"
...
lynchrate_lm <- lm_robust(I(lynchrate * 100) ~ march + pslave1860 + land.ineq1860 + log(acimp1860) + log(fvalpac1860) + rail1860 + log(totpop1860)+ state.abb, data = county_data)


names_lm_1930 <- lm_robust(I(share_southern_names_1930 * 100) ~ march + pslave1860 + land.ineq1860 + log(acimp1860) + log(fvalpac1860) + rail1860 + log(totpop1860) + state.abb, data = county_data)

names_lm_1880 <- lm_robust(I(share_southern_names_1880 * 100) ~ march + pslave1860 + land.ineq1860 + log(acimp1860) + log(fvalpac1860) + rail1860 + log(totpop1860)+ state.abb, data = county_data)
...
modelsummary(list("First names (1880)" = names_lm_1880, "First names (1930)" = names_lm_1930,
             ...
             title = "Other outcomes - OLS results", label = "tab:other_outcomes_ols_all", 
             coef_omit = "^state", gof_map = gm,
             stars = TRUE, output = "tables/other_outcomes_ols_all.tex")
...
names_iv_1930 <- iv_robust(as.formula(str_c("I(share_southern_names_1930 * 100) ~", 'march', exo_vars, "|sl_inst", exo_vars)),
                          data = county_data) 


names_iv_1880 <- iv_robust(as.formula(str_c("I(share_southern_names_1880 * 100) ~", 'march', exo_vars, "|sl_inst", exo_vars)),
                          data = county_data) 
...
#    out <- tibble::tibble(`Dep. variable mean:` = dv)
#    return(out)
#}
...
modelsummary(list("First names (1880)" = names_iv_1880, "First names (1930)" = names_iv_1930,
             ...
             title = "Other outcomes - IV - second stage", label = "tab:other_outcomes_iv_all_ss", 
             coef_omit = "^state", gof_map = gm,
             stars = TRUE, output = "tables/other_outcomes_iv_all_ss.tex")
