...
names_by_county <- read_csv('data/full count census/southern_first_names_by_county_1930.csv')

county_data <- county_data %>% 
  mutate(fips_char = str_pad(fips, 5, pad = "0")) %>% 
  left_join(names_by_county, by = c('fips_char' = 'fips')) #%>% View()



county_data <- county_data %>% 
  right_join(counties, by = c("fips")) %>%
  mutate(share_southern_names_1930 =(first_name_jefferson + first_name_robert_e + first_name_robert_l + first_name_stonewall)/n_obs) 
...
urb_pop <- urb_pop %>% 
  mutate(fips = as.numeric(str_c(str_sub(STATEA, start = 1), 
                                 str_sub(COUNTYA, start = 2, end = -2)))) %>%filter(STATE == 'Georgia')# %>%   filter(fips == 13003)
